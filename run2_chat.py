# ==========================================================
# 【run6】庄子对话 —— 用微调好的模型聊天
# 加载 run5 训出的 zhuangzi-sft.pth，像跟庄子对话一样聊天。
#
# 和 run5 配套（训练 / 使用 已分开）:
#   ① 先跑 run1_sft.py 训练出权重 zhuangzi-sft.pth
#   ② 再跑本文件 → 加载权重，跟庄子对话
#
# 需要的权重: zhuangzi-sft.pth（本文件夹，由 run5 生成）
# 说明: 数据是中文庄子问答，模型会用文言/庄子腔回你。
#       输入用现代白话问即可（如「我很焦虑怎么办」），它会用庄子的话接。
#
# 依赖: transformers（加载 uer tokenizer），用 D:\\envs\\zhuangzi 环境跑
# ==========================================================
